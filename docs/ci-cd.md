...
The container publisher pushes the canonical `v...` tag, captures its registry manifest digest, and signs that digest with keyless Cosign using the release workflow's GitHub OIDC identity. Cosign writes its OCI signature objects to the sibling `ghcr.io/<owner>/<repository>-signatures` package, keeping non-runnable metadata out of the main image package's latest-version display. The publisher immediately verifies the signature against the exact repository, workflow, tag ref, and `https://token.actions.githubusercontent.com` issuer. Only after verification succeeds does it publish the bare-version alias and, for stable releases, `latest`; every alias digest must match the signed digest. Podman and Cosign share an ephemeral Docker-compatible registry authentication file under the runner's temporary directory. The job receives `packages: write` for GHCR and `id-token: write` for the short-lived signing certificate. It stores no long-lived signing key and does not map the environment's Cloudflare secret into its steps.
...
The coverage workflow runs the browser and server Vitest projects for pushes and pull requests. Its unprivileged collection job retains the LCOV report as a short-lived artifact without access to an OIDC token. For pushes and same-repository pull requests, a separate upload job checks out a trusted revision, downloads only that report, and publishes it to Codecov with GitHub OIDC; the upload job is skipped for forks. Activate this public repository in Codecov before requiring the workflow; no long-lived `CODECOV_TOKEN` secret is used. The README badge reports coverage from `main`.
...
The first signed release creates the `kite-signatures` GHCR package. Make that package public so users can verify the public `kite` image without registry credentials, and confirm that it is connected to this repository with Actions write access. The release job's immediate verification is authenticated and can succeed before public visibility is configured, but anonymous consumer verification cannot.
