	...
	return {
		...
		hydrate() {
			...
			try {
				...
				const value: unknown = JSON.parse(current ?? localStorage.getItem(LEGACY_AZURE_SESSIONS_STORAGE_KEY) ?? '[]');
		...
		remove(id) {
			...
			profiles = next;
			if (browser)
				window.dispatchEvent(new CustomEvent('kite:azure-authentication-profile-removed', { detail: { id } }));
		},
		bindAccount(id, account) {
			const next = profiles.map((profile) => (profile.id === id ? { ...profile, account } : profile));
			persist(next);
			profiles = next;
		},
		clearAccount(id) {
